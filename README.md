...
## Решение демонстрационного экзамена "Сетевой и системный администратор" (ALT Linux)

**Описание:** Решение задач, поставленных в техническом задании, для системы ALT Linux. Учитываются требования по использованию DHCP, блокировке сайтов через `/etc/hosts` и созданию пользователя `resu`.

**Предварительные действия:**

1.  **Загрузка и запуск ALT Linux с графическим интерфейсом на компьютере BR-AD.**
2.  **Открытие терминала.**
3.  **Получение прав суперпользователя (root).** `su -` (и ввод пароля root). Использовать `sudo` при необходимости (например, `sudo systemctl restart network`), если нет пароля root или текущий пользователь не входит в группу `wheel`/`sudoers`.
4.  **Установка `mc` (Midnight Commander):**
    ```bash
    apt-get update
    apt-get install mc
    ```
...
1.  **Настройка DHCP на интерфейсе `enp0s3` (ранее `esn192`):**

    *   **Определение имени интерфейса:** `ip addr` или `ifconfig -a`
    *   **Использование `nmtui`:**
        ```bash
        nmtui
        ```
        *   "Edit a connection" -> `enp0s3`
...
3.  **Создание "особого" файла с информацией:**
...
1.  **Постоянная установка операционной системы:**
...
2.  **Постоянный выход из системы (автоматический выход должен быть в 20:30):**
...
5.  **Отсутствие прав на редактирование файлов в папке `/var` у пользователя `resu`:**
    ...
    *   **Предупреждение: ПРЕДОСТАВЛЕНИЕ ПРАВ НА /VAR ЯВЛЯЕТСЯ ПЛОХОЙ ПРАКТИКОЙ С ТОЧКИ ЗРЕНИЯ БЕЗОПАСНОСТИ! ДЕЙСТВУЙТЕ ОСТОРОЖНО!**
...
*   **Безопасность:**  Избегайте предоставления ненужных прав.
*   **Документация:**  Обращайтесь к официальной документации ALT Linux.
*   **Резервные копии:**  Перед внесением изменений создавайте резервные копии.
*   **Тестирование:**  Тщательно проверяйте работоспособность после каждой задачи.
